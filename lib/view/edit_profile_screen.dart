...
class _EditProfileState extends State<EditProfile> {
  ...
  // void checkValues() {
  //   if (fullNameController.text.isEmpty &&phoneController.text.isEmpty &&imageFile == null) {
  //     print("Please fill all the fields");
  //     CustomDialog.showAlertDialog(context, "Incomplete Changes",
  //         "Please update your profile before submitting.");
  //   } else {
  //     log("Uploading data..");
  //     uploadData();
  //     CustomDialog.showAlertDialog(context, "Update Profile",
  //         "Successfully Update your profile...");
  //   }
  // }
  void checkValues() {
    if (fullNameController.text.isEmpty &&  imageFile == null) {
      // No changes made, you can just navigate back or show a message
  ...
  //     await AppPreferences.setPhone(phoneController.text.trim());
  ...
  Future uploadData() async {
    ...
    if (uid != null) {
      ...
      if (imageFile != null || fullNameController.text.trim() != fullname) {
        // At least one field has changed, proceed with updates

        // Handle image upload and URL retrieval only if the image has changed
        String? imageUrl;
        if (imageFile != null) {
          UploadTask uploadTask = FirebaseStorage.instance
              .ref("profilepictures")
          ...
          imageUrl = profilePic; // Reuse the existing URL if the image hasn't changed
        }
        ...
        // Update the user's data in Firebase and locally
        await CommonMethod.saveUserData(userModel);
        await AppPreferences.setFullName(fullNameController.text.trim());
        //await AppPreferences.setPhone(phoneController.text.trim());
        await AppPreferences.setProfilePic(imageUrl!);
                ...
                GestureDetector(
                  ...
                  child: Stack(
                    children: [
                      ...
                      //         (imageFile != null) ? FileImage(imageFile!) :   profilePic!,
                      //
                      //     )),
                      //   ),
                      // ),
                      SizedBox(
                        ...
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(100),
                          child: (imageFile != null) // Check if a local image file exists
                              ? Image.file(imageFile!) // Display the local image file
                              : Image.network(profilePic!), // Display the network image
                        ),
                      ),
